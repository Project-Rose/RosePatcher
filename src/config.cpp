...
ConfigMenuOpenedCallback(WUPSConfigCategoryHandle rootHandle) {
  ...
  try {
    ...
    if (!utils::isJapanConsole()) {
      ...
      root.add(
          WUPSConfigItemStub::Create("Note: Wii U Menu will restart if \"Add "
                                     "TVii Icon to the Wii U Menu\""));
...
void InitializeConfig() {
  ...
  // For when we can't detect someones console region and their console region
  // is actually Japan
  if ((storageRes = WUPSStorageAPI::GetOrStoreDefault(
           FORCE_JPN_CONSOLE_CONFIG_ID, forceJPNconsole,
           FORCE_JPN_CONSOLE_DEFAULT_VALUE)) != WUPS_STORAGE_ERROR_SUCCESS) {
    DEBUG_FUNCTION_LINE("GetOrStoreDefault failed: %s (%d)",
                        WUPSStorageAPI_GetStatusStr(storageRes), storageRes);
  }
