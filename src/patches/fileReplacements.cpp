...
DECL_FUNCTION(FSStatus, FSOpenFile_VINO, FSClient *client, FSCmdBlock *block,
  ...
  if (config::connectToRose && strcmp(VINO_CONFIG_PATH, path) == 0) {
    FSStatus res =
        real_FSOpenFile_VINO(client, block, path, mode, handle, errorMask);
...
DECL_FUNCTION(FSStatus, FSReadFile_VINO, FSClient *client, FSCmdBlock *block,
  ...
  if (config_handle && *config_handle == handle) {
    DEBUG_FUNCTION_LINE(
        "Trying to read vino config detected, returning patched data.");
    strlcpy((char *)buffer, (const char *)rose_config_txt, size * count);
    return (FSStatus)count;
  }

  return real_FSReadFile_VINO(client, block, buffer, size, count, handle, unk1,
                              flags);
}

DECL_FUNCTION(FSStatus, FSCloseFile_VINO, FSClient *client, FSCmdBlock *block,
...
WUPS_MUST_REPLACE_FOR_PROCESS(FSOpenFile_VINO, WUPS_LOADER_LIBRARY_COREINIT,
                              FSOpenFile, WUPS_FP_TARGET_PROCESS_TVII);
WUPS_MUST_REPLACE_FOR_PROCESS(FSReadFile_VINO, WUPS_LOADER_LIBRARY_COREINIT,
                              FSReadFile, WUPS_FP_TARGET_PROCESS_TVII);
WUPS_MUST_REPLACE_FOR_PROCESS(FSCloseFile_VINO, WUPS_LOADER_LIBRARY_COREINIT,
                              FSCloseFile, WUPS_FP_TARGET_PROCESS_TVII);
