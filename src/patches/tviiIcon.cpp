...
void perform_men_patches(bool enable) {
  ...
  if (!utils::is555OrHigher()) {
    DEBUG_FUNCTION_LINE(
        "Console not version 5.5.5 or greater, skipping men patches");
    return;
  }
  ...
  if (utils::patch::find_mem(men_rpx.dataAddr, men_rpx.dataSize, root_rpx_check,
                             sizeof(root_rpx_check))) {
    DEBUG_FUNCTION_LINE("perform_men_patches: men.rpx has been replaced by "
                        "root.rpx, skipping patches ...");
    return;
  }

  if (enable) {
    DEBUG_FUNCTION_LINE("perform_men_patches: enabling tvii patches");
    utils::patch::patch_instruction((uint8_t *)men_rpx.textAddr + 0x1e0b10,
                                    0x5403d97e,
                                    0x38600001); // v277
    utils::patch::patch_instruction((uint8_t *)men_rpx.textAddr + 0x1e0a20,
                                    0x5403d97e,
                                    0x38600001); // v257
  } else {
...
void perform_hbm_patches(bool enable) {
  ...
  if (!utils::is555OrHigher()) {
    DEBUG_FUNCTION_LINE(
        "Console not version 5.5.5 or greater, skipping hbm patches");
    return;
  }
  ...
  if (enable) {
    ...
    utils::patch::patch_instruction((uint8_t *)hbm_rpx.textAddr + 0x0ec430,
                                    0x5403d97e,
                                    0x38600001); // v197
    utils::patch::patch_instruction((uint8_t *)hbm_rpx.textAddr + 0x0ec434,
                                    0x7c606110,
                                    0x38600001); // v180
  } else {
...
DECL_FUNCTION(int, FSOpenFile_TVII, FSClient *pClient, FSCmdBlock *pCmd,
...
WUPS_MUST_REPLACE_FOR_PROCESS(FSOpenFile_TVII, WUPS_LOADER_LIBRARY_COREINIT,
