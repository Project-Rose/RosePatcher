...
void addCertificateToWebKit() {
  ...
  if (ret != OS_DYNLOAD_OK || libwkc == 0) {
    DEBUG_FUNCTION_LINE("OSDynLoad_Acquire failed to get libwkc.rpl in tvii. "
                        "(Most likely not loaded). With error: %d",
                        ret);
    return;
  }

  if (OSDynLoad_FindExport(libwkc, OS_DYNLOAD_EXPORT_FUNC,
                           "WKCWebKitSSLRegisterRootCAByDER__3WKCFPCci",
                           (void **)&WKC_SSLRegisterRootCAByDER) !=
      OS_DYNLOAD_OK) {
    DEBUG_FUNCTION_LINE(
        "FindExport setDeveloperExtrasEnabled__Q2_3WKC11WKCSettingsFb failed.");
    return;
  }
...
DECL_FUNCTION(NSSLError, NSSLAddServerPKI, NSSLContextHandle context,
...
WUPS_MUST_REPLACE_FOR_PROCESS(NSSLAddServerPKI, WUPS_LOADER_LIBRARY_NSYSNET,
                              NSSLAddServerPKI, WUPS_FP_TARGET_PROCESS_TVII);